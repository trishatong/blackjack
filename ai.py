...
class Agent:
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        # states = inside simulator; self.simulator = Game()
        # reward = game file - check reward
        ...
        for simulation in range(num_simulation):
            ...
            s = []
            r = []
            s.append(self.simulator.state)
            r.append(self.simulator.check_reward())
            while not self.simulator.game_over():
                curr = self.simulator.state
                action = self.default_policy(curr)
                state, reward = self.make_one_transition(action)
                s.append(state)
                r.append(reward)

            for state, reward in zip(s[::-1], r[::-1]):
                G = reward + (DISCOUNT * G)
                self.S_MC[state] += G
                self.N_MC[state] += 1
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while not self.simulator.game_over():
                ...
                self.TD_values[curr] = self.TD_values[curr] + self.alpha(self.N_TD[curr]) * (reward + DISCOUNT * (self.TD_values[next_state] - self.TD_values[curr]))
                self.N_TD[curr] += 1
                if self.simulator.game_over():
                    self.TD_values[next_state] = next_reward
                    self.N_TD[next_state] += 1
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while not self.simulator.game_over():
                curr = self.simulator.state
                action = self.pick_action(curr, epsilon)
                reward = self.simulator.check_reward()
                next_state, next_reward = self.make_one_transition(action)
                next_action = self.pick_action(next_state, epsilon)
                self.Q_values[curr][action] = self.Q_values[curr][action] + self.alpha(self.N_Q[curr][action]) * (reward + DISCOUNT * (self.Q_values[next_state][next_action] - self.Q_values[curr][action]))
                self.N_Q[curr][action] += 1
                if self.simulator.game_over():
                    self.Q_values[next_state][next_action] = next_reward
                    self.N_Q[next_state][next_action] += 1
    ...
    def pick_action(self, s, epsilon):
        ...
        if random.random() < epsilon:
            ...
            best_action = self.Q_values[s].index(max(self.Q_values[s]))
            return best_action
